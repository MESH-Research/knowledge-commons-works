...
The payload formatting function receives all the same arguments that are
returned by the RDMRecordsService method that was called. This includes the
record data, the record object, and the identity of the user who triggered the
event. The payload function can also receive any additional keyword arguments
that are passed to the RDMRecordsService method. For example, the
``update_draft`` event is triggered by the ``update_draft`` method of the
RDMRecordsService. This method takes a ``revision_id`` keyword argument, which
is passed to the payload function.
