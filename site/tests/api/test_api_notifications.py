...
def test_notify_for_request_acceptance(
    ...
    with app.test_client() as client:
        ...
        response = logged_in_client.put(
            f"{app.config['SITE_API_URL']}/api/records/{draft_id}/draft/"
            "review",
        ...
        submit_url = request_data["links"]["actions"]["submit"]
        ...
        # response = logged_in_client.post(
        #     f"{app.config['SITE_API_URL']}/api/requests/{request_id}/actions/submit",
        ...
        #     f"{app.config['SITE_API_URL']}/api/requests/{request_id}/actions/accept",
        ...
        app.logger.warning(f"mailbox: {mailbox[0].body}")
        app.logger.warning(f"mailbox: {mailbox[0].html}")
        ...
        # TODO: Test overridden templates
        app.logger.warning(f"mailbox: {mailbox[1].body}")
        app.logger.warning(f"mailbox: {mailbox[1].html}")
        ...
        assert json.loads(
            ...
            {
                "request_id": request_id,
                "notification_type": "community-submission.accept",
...
def test_notify_for_request_decline(
    ...
    with app.test_client() as client:
        ...
        # response = client.post(
        #     f"{app.config['SITE_API_URL']}/api/requests/{request_id}/actions/decline",
        #     data=json.dumps(accept_body),
        #     headers={
        #         **headers,
        #         "Authorization": f"Bearer {admin.allowed_token}",
        ...
        assert json.loads(
            ...
            {
                "request_id": request_id,
                "notification_type": "community-submission.decline",
                "request_type": "community-submission",
...
def test_notify_for_request_cancellation(
    ...
    with app.test_client() as client:
        ...
        # response = client.post(
        #     f"{app.config['SITE_API_URL']}/api/requests/{request_id}/actions/cancel",
...
def test_notify_for_new_request_comment(
    ...
    with app.test_client() as client:
        ...
        # response = client.post(
        #     f"{app.config['SITE_API_URL']}/api/requests/{request_id}/comments",
...
def test_read_unread_notifications_by_view(
    ...
    with app.test_client() as client:
        ...
        response = client.get(
            f"{app.config['SITE_API_URL']}/api/users/{user.id}/"
        ...
        response = client.get(
            f"{app.config['SITE_API_URL']}/api/users/{admin_id}/"
            "notifications/unread/list"
        )
...
def test_clear_unread_notifications_by_view(
    ...
    with app.test_client() as client:
        ...
        response = client.get(
            f"{app.config['SITE_API_URL']}/api/users/{current_user.id}/"
...
def test_unread_endpoint_bad_methods(running_app, db, client, admin):
    """
    ...
    with app.test_client() as client:
        ...
        response = client.post(
            f"{app.config['SITE_API_URL']}/api/users/5/notifications/"
            "unread/list",
            data=json.dumps({}),
        )
        ...
        assert response.status_code == 400  # FIXME: This should be 405
        assert json.loads(response.data) == {
            "message": "CSRF token missing or incorrect.",
            "status": 400,  # FIXME: This should be 405
        }
