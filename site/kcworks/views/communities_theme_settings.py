...
from kcworks.services.communities.default_branding import branding_theme_style
...
def communities_settings_theme(pid_value, community, community_ui):
    ...
    record = current_communities.service.record_cls.pid.resolve(pid_value)
    style = branding_theme_style(record)
    theme_meta = record.get("theme") or {}

    default_theme = {
        "enabled": True,
        "style": style,
    }
    if theme_meta.get("autogeneratedLogo") is not None:
        default_theme["autogeneratedLogo"] = theme_meta["autogeneratedLogo"]
