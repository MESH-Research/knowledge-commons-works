...
// (primaryColor, primaryTextColor, mainHeaderBackgroundColor). The
// invenio_communities community_theme_template.css applies the header
// tint and link color from those persisted theme values, so the per-page
// JS tint code that lived here is now redundant.
...
const CommunityDetailsHeader = ({
  ...
  communityType,
  customFields,
  dashboardCommunityOptIn,
  dashboardEnabledCommunity,
  ...
  let dashboardEnabled = communityStatsEnabled && dashboardEnabledCommunity;
  ...
  const dropdown_default_item = dropdown_items.find((item) => item.key === activeMenuItem);
      ...
      <div className="row pb-0 community-header">
        <div className="sixteen wide mobile three wide tablet two wide computer column pt-5">
          <div className="ui rounded image community-image community-header-logo">
            <Image
              src={communityLogoUrl || "/static/images/square-placeholder.png"}
            ...
            {communityType && (
              <div className="row collection-type">
                <div className="column">
                  ...
                  <span className="label label-keyword">{communityType}</span>
                </div>
              </div>
            )}
                ...
                <div className="column">
                  {organizations.map((org, idx) => (
                    <div className="inline-computer mt-5 org" key={idx}>
                      {idx == 0 && <i className="building outline icon" aria-hidden="true"></i>}
                      {org.name}
                      {org.id && (
                        <a
...
//   const canModerate = headerContainer.dataset.canModerate === "True" ? true : false;
//   const canRead = headerContainer.dataset.canRead === "True" ? true : false;
//   const canSearchRequests = headerContainer.dataset.canSearchRequests === "True" ? true : false;
//   const canUpdate = headerContainer.dataset.canUpdate === "True" ? true : false;
//   const communityLogoUrl = headerContainer.dataset.communityLogoUrl;
//   const communityStatsEnabled =
//     headerContainer.dataset.communityStatsEnabled === "True" ? true : false;
//   const communityTitle = headerContainer.dataset.communityTitle;
//   const communityType = headerContainer.dataset.communityType;
//   const customFields = JSON.parse(headerContainer.dataset.customFields || "{}");
//   const dashboardCommunityOptIn =
//     headerContainer.dataset.dashboardCommunityOptIn === "True" ? true : false;
//   const dashboardEnabledCommunity =
//     headerContainer.dataset.dashboardEnabledCommunity === "True" ? true : false;
//   const hasCurationPolicyContent = headerContainer.dataset.hasCurationPolicyContent;
...
//         communityStatsEnabled={communityStatsEnabled}
//         communityTitle={communityTitle}
//         communityType={communityType}
//         customFields={customFields}
//         dashboardCommunityOptIn={dashboardCommunityOptIn}
//         dashboardEnabledCommunity={dashboardEnabledCommunity}
...
document.addEventListener("DOMContentLoaded", () => {
  const mobileMenu = document.querySelector(".community-header-mobile-menu");
  if (mobileMenu) {
