...
class PerFieldEditPermissionsComponent(ServiceComponent):
    ...
    def _separate_conditional_fields(
        ...
        for restricted_field in restricted_fields:
            ...
            current_app.logger.info(f"Bracket section match: {bracket_section_match}")
        ...
        current_app.logger.info(f"Simple restricted fields: {simple_restricted_fields}")
    ...
    def update_draft(
        ...
        **kwargs,
        ...
        if record.is_published and record.parent:
            community_config: (
                dict | None
            ) = PerFieldEditPermissionsComponent._get_permissions_config(
            ...
            if community_config:
                ...
                for field, key in changed_restricted_fields:
                    ...
                    current_app.logger.info(
                        f"Community field policy allows: "
                        f"{community_field_policy.allows(identity)}"
                    )
                    current_app.logger.info(
                    ...
                    if not community_field_policy.allows(identity):
                        current_app.logger.info(
                            "Community policy disallows editing restricted fields"
                        )
                        errors.append(
                            {
                                "field": field,
