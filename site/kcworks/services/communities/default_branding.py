"""Default-branding service component for communities.

Wires the [`kcworks.services.geopattern`][kcworks.services.geopattern] port
into the `invenio-communities` create/update lifecycle so that every
freshly-created community has, by the time the user sees its page:

- A stored geopattern PNG in `record.files["logo"]`.
- Slug-derived theme fields on `record["theme"]["style"]`:
  `primaryColor`, `primaryTextColor`, `secondaryColor`,
  `secondaryTextColor`, `mainHeaderBackgroundColor`.
- Site-neutral theme colors for user-uploaded logos from
  `KCWORKS_COMMUNITIES_GENERIC_THEME_COLORS` (including `tertiaryColor`).
- Default header presentation flags: geopattern logos use
  `mainHeaderUseLogo=true` and `mainHeaderUseGradient=true`; custom logos
  use `mainHeaderUseLogo=false` and `mainHeaderUseGradient=true`.
- `theme.autogeneratedLogo` set to `True` when the logo is the slug-derived
  geopattern default, or `False` after a user upload.

The component runs synchronously inside the create request so the typical
case is "everything is set before the redirect". If the inline pipeline
fails (cairosvg can't render, Pillow blows up, ...), the failure is logged
and a Celery task ([`generate_default_branding`]
[kcworks.services.communities.default_branding.generate_default_branding])
is enqueued to retry the full pipeline outside the request.

The component is also dispatched on `delete_logo` (via the patched
[`CommunityService.delete_logo`][])
so removing the custom logo re-applies slug-derived colors and the
default header presentation flags.

This module is also the target of the
`invenio_celery.tasks` entry point declared in `pyproject.toml`.
"""

from __future__ import annotations

import hashlib
from io import BytesIO
from typing import Any

from flask import current_app
from invenio_records_resources.services.records.components import ServiceComponent

from .tasks import generate_default_branding

KCWORKS_COMMUNITIES_GENERIC_THEME_COLORS = "KCWORKS_COMMUNITIES_GENERIC_THEME_COLORS"

_AUTOGENERATED_HEADER_STYLE: dict[str, bool] = {
    "mainHeaderUseLogo": True,
    "mainHeaderUseGradient": True,
}

_GENERIC_HEADER_STYLE: dict[str, bool] = {
    "mainHeaderUseLogo": False,
    "mainHeaderUseGradient": True,
}


def generic_theme_colors() -> dict[str, str]:
    """Return the site-neutral theme color palette from app config.

    Returns:
        Hex color strings from ``KCWORKS_COMMUNITIES_GENERIC_THEME_COLORS``.

    Raises:
        RuntimeError: If the config variable is unset or empty.
    """
    colors = current_app.config.get(KCWORKS_COMMUNITIES_GENERIC_THEME_COLORS)
    if not colors:
        raise RuntimeError(
            f"{KCWORKS_COMMUNITIES_GENERIC_THEME_COLORS} must be set in the "
            "Invenio application config"
        )
    return dict(colors)


def default_theme_style(slug: str) -> dict[str, str | bool]:
    """Return the full default `theme.style` for a community.

    Slug-derived colors come from
    [`derive_theme_colors`][kcworks.services.geopattern.derive_theme_colors];
    header presentation flags are fixed service defaults.

    Args:
        slug: Community slug.

    Returns:
        Default values for all keys seeded by default branding.
    """
    from kcworks.services.geopattern import derive_theme_colors

    return {**derive_theme_colors(slug), **_AUTOGENERATED_HEADER_STYLE}


def generic_theme_style() -> dict[str, str | bool]:
    """Return a site-neutral `theme.style` for custom-uploaded logos.

    Returns:
        Generic theme style values (not slug-derived).
    """
    return {**generic_theme_colors(), **_GENERIC_HEADER_STYLE}


def theme_style_differs_from_defaults(record: Any) -> list[str]:
    """List `theme.style` keys whose values differ from service defaults.

    Args:
        record: A Community record.

    Returns:
        Keys in the default style dict that are missing or not equal to the
        expected default for this record's logo type.
    """
    defaults = branding_theme_style(record)
    style = (record.get("theme") or {}).get("style") or {}
    return [key for key, value in defaults.items() if style.get(key) != value]


def missing_theme_style_keys(record: Any) -> list[str]:
    """List default `theme.style` keys that are not set on `record`.

    Args:
        record: A Community record.

    Returns:
        Keys present in the service default style dict but absent from
        `record["theme"]["style"]`.
    """
    defaults = branding_theme_style(record)
    style = (record.get("theme") or {}).get("style") or {}
    return [key for key in defaults if key not in style]


def _stored_logo_sha256(record: Any) -> str | None:
    """Return the SHA-256 digest of the community logo bytes, if present."""
    logo = record.files.get("logo")
    if logo is None:
        return None
    with logo.open_stream("rb") as fp:
        return hashlib.sha256(fp.read()).hexdigest()


def _expected_geopattern_logo_sha256(slug: str) -> str:
    """Return the SHA-256 digest of the default geopattern logo for `slug`."""
    from kcworks.services.geopattern import to_png

    return hashlib.sha256(to_png(slug, width=480, height=480)).hexdigest()


def is_autogenerated_logo(record: Any) -> bool:
    """Return whether the community logo is (or will be) the geopattern default.

    Uses `theme.autogeneratedLogo` when set. When the flag is absent, infers
    from a byte-for-byte match against the slug-derived geopattern PNG. A
    missing logo is treated as autogenerated because create/backfill will
    generate one.

    Args:
        record: A Community record.

    Returns:
        `True` when slug-derived theme colors are appropriate for the logo.
    """
    theme = record.get("theme") or {}
    flag = theme.get("autogeneratedLogo")
    if flag is True or flag is False:
        return flag
    if record.files.get("logo") is None:
        return True
    stored = _stored_logo_sha256(record)
    if stored is None:
        return True
    try:
        return stored == _expected_geopattern_logo_sha256(record.slug)
    except Exception:
        current_app.logger.exception(
            "Could not compare logo to geopattern default for community %s",
            getattr(record, "id", "<unknown>"),
        )
        return False


def branding_theme_style(record: Any) -> dict[str, str | bool]:
    """Return the appropriate default `theme.style` for `record`.

    Slug-derived colors apply when the logo is (or will be) the geopattern
    default; otherwise a fixed site-neutral palette is used.

    Args:
        record: A Community record.

    Returns:
        Default style key/value pairs for branding seed and backfill.
    """
    if is_autogenerated_logo(record):
        return default_theme_style(record.slug)
    return generic_theme_style()


def apply_default_branding(
    record: Any,
    *,
    force_logo: bool = False,
    force_theme: bool = False,
    skip_logo: bool = False,
    skip_theme: bool = False,
) -> None:
    """Set the geopattern logo and theme colors on `record`.

    Pure data-mutation: writes `record.files["logo"]` (a fresh `BytesIO`
    of the slug-derived PNG) and ensures the default theme.style keys are
    populated. Idempotent for the theme half; respects existing values
    unless `force_theme=True`. For the logo, only writes when missing
    unless `force_logo=True`.

    Args:
        record: A `Community` record. Must already have a `slug` and a
            stable `id`; call this after `PIDComponent` and
            `OwnershipComponent` have run.
        force_logo: When `True`, overwrite any existing logo file. Used
            from `delete_logo` to re-seed branding after a removal.
        force_theme: When `True`, overwrite any existing theme keys even
            if the admin previously set them. Used from `delete_logo`.
        skip_logo: When `True`, do not write or regenerate the logo file.
            Used by the backfill CLI's ``--theme-only`` mode.
        skip_theme: When `True`, do not seed or update theme keys. Used by
            the backfill CLI's ``--logo-only`` mode.
    """
    # Imported lazily so the geopattern module's cairosvg/Pillow imports
    # don't fire during module-load of every Flask request.
    from kcworks.services.geopattern import to_png

    slug = record.slug
    will_write_logo = not skip_logo and (
        force_logo or record.files.get("logo") is None
    )

    if not skip_theme:
        if force_logo:
            record.setdefault("theme", {})["autogeneratedLogo"] = True
        elif record.get("theme", {}).get("autogeneratedLogo") is None:
            record.setdefault("theme", {})["autogeneratedLogo"] = is_autogenerated_logo(
                record
            )

        theme_style = (
            default_theme_style(slug)
            if force_logo
            else branding_theme_style(record)
        )

        # 1) Theme: only set missing keys unless `force_theme`. Mutating the
        # record's `theme` dict directly bypasses the `set_theme` permission
        # check that `CommunityThemeComponent` would apply to wire-supplied
        # data. That's the intent: we're seeding defaults from server code,
        # not honoring user input.
        record.setdefault("theme", {})
        style = record["theme"].setdefault("style", {})
        wrote_any_theme = False
        for key, value in theme_style.items():
            if force_theme or key not in style:
                style[key] = value
                wrote_any_theme = True
        if wrote_any_theme:
            record["theme"].setdefault("enabled", True)

    # 2) Logo: only generate when missing if forced. PNG bytes go
    # through `BytesIO` so the underlying files manager treats them as a
    # stream. `to_png` is the slowest call in this path (~50-300 ms in
    # practice for a 480x480 render); doing it after the early-return
    # avoids paying that cost for communities that already have a logo.
    if not will_write_logo:
        return
    png_bytes = to_png(slug, width=480, height=480)
    record.files["logo"] = BytesIO(png_bytes)
    record.setdefault("theme", {})["autogeneratedLogo"] = True


class DefaultBrandingComponent(ServiceComponent):
    """Set geopattern logo + slug-derived theme colors on community lifecycle.

    Behavior:

    - `create`: always runs. Writes both the logo and the default theme
      style keys. Any inline failure is logged and a Celery retry task is
      enqueued; the create itself never aborts because of branding.
    - `update`: tops up missing slug-derived color keys only. Header
      presentation flags are seeded on create and reset on `delete_logo`,
      not re-applied when absent from a user save.
    - `update_logo`: marks `theme.autogeneratedLogo` as `False` after a
      user upload.
    - `delete_logo`: re-runs the full pipeline with `force_logo=True`
      and `force_theme=True`, treating the explicit deletion as a
      request to revert to the full default theme style.

    Register this component AFTER `CommunityThemeComponent` in
    `COMMUNITIES_SERVICE_COMPONENTS` so user-supplied themes from
    `data["theme"]` win and we only fill in what's still missing.
    """

    def create(
        self,
        identity: Any,
        data: dict[str, Any] | None = None,
        record: Any = None,
        **kwargs: Any,
    ) -> None:
        """Apply default branding to a freshly-created community."""
        if record is None:
            return
        try:
            apply_default_branding(record)
        except Exception:
            current_app.logger.exception(
                "Default branding inline failed for community %s; "
                "falling back to Celery",
                getattr(record, "id", "<unknown>"),
            )
            try:
                generate_default_branding.delay(str(record.id))
            except Exception:
                current_app.logger.exception(
                    "Default branding Celery enqueue failed for %s",
                    getattr(record, "id", "<unknown>"),
                )

    def update(
        self,
        identity: Any,
        data: dict[str, Any] | None = None,
        record: Any = None,
        **kwargs: Any,
    ) -> None:
        """Top up missing theme colors; never touches the logo."""
        if record is None:
            return
        try:
            theme = record.get("theme") or {}
            style = theme.get("style") or {}
            record.setdefault("theme", {})
            if record["theme"].get("autogeneratedLogo") is None:
                record["theme"]["autogeneratedLogo"] = is_autogenerated_logo(record)
            missing = [
                key
                for key in branding_theme_style(record)
                if key not in style
            ]
            if not missing:
                return
            defaults = branding_theme_style(record)
            record["theme"].setdefault("style", {}).update({
                k: defaults[k] for k in missing
            })
            record["theme"].setdefault("enabled", True)
        except Exception:
            current_app.logger.exception(
                "Default theme top-up failed for community %s",
                getattr(record, "id", "<unknown>"),
            )

    def update_logo(
        self,
        identity: Any,
        record: Any = None,
        **kwargs: Any,
    ) -> None:
        """Mark the logo as user-uploaded after `CommunityService.update_logo`."""
        if record is None:
            return
        record.setdefault("theme", {})["autogeneratedLogo"] = False

    def delete_logo(
        self,
        identity: Any,
        record: Any = None,
        deleted_file: Any = None,
        **kwargs: Any,
    ) -> None:
        """Re-seed default branding after the explicit logo-delete endpoint.

        Invoked from the patched
        [`CommunityService.delete_logo`][invenio_communities.communities.services.service.CommunityService.delete_logo]
        after the old logo file has been popped and physically deleted,
        and before its `RecordCommitOp(record)` is registered with the
        UoW. We mutate `record.files["logo"]` and `record["theme"]` here;
        the outer service method commits both in the same UoW.
        """
        if record is None:
            return
        try:
            apply_default_branding(record, force_logo=True, force_theme=True)
        except Exception:
            current_app.logger.exception(
                "Default branding regeneration failed for community %s on "
                "delete_logo; enqueueing Celery retry",
                getattr(record, "id", "<unknown>"),
            )
            try:
                generate_default_branding.delay(str(record.id))
            except Exception:
                current_app.logger.exception(
                    "Default branding Celery enqueue failed for %s",
                    getattr(record, "id", "<unknown>"),
                )
