...
respective operations. By default, admin-customized theme keys survive
backfill (only missing keys are filled). Pass ``--reset-theme`` to
overwrite existing values with current service defaults.
"""
...
def three_communities(minimal_community_factory, search_clear):
    ...
    Args:
        ...
        search_clear: Ensures search indices are cleared for the test module.
...
def test_backfill_theme_only_skips_logo(app, db, three_communities) -> None:
    ...
    assert style.get("secondaryColor") == expected["secondaryColor"]
    assert style.get("secondaryTextColor") == expected["secondaryTextColor"]
    assert (
        style.get("mainHeaderBackgroundColor") == expected["mainHeaderBackgroundColor"]
    )
...
def test_backfill_preserves_admin_theme_value(
    ...
    style = record["theme"]["style"]
    assert style["primaryColor"] == "#abcdef"
...
def test_backfill_resets_admin_theme_to_defaults(
    app,
    db,
    minimal_community_factory,
    search_clear,
    ...
    record = _read_record(community.id)
    style = record["theme"]["style"]
    expected = default_theme_style("branded-admin-custom")
    for key in expected:
...
def test_backfill_uses_generic_theme_for_custom_logo(
    ...
    for key in GENERIC_THEME_KEYS:
        assert style[key] == expected[key]
    assert record["theme"].get("autogeneratedLogo") is False


def test_backfill_reset_theme_preserves_custom_logo_flag(
    app,
    db,
    minimal_community_factory,
    search_clear,
