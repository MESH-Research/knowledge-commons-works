...
def test_create_sets_logo_and_theme(running_app, db, branded_community) -> None:
    ...
    assert record["theme"].get("autogeneratedLogo") is True
    assert style["mainHeaderUseLogo"] is True
    assert style["mainHeaderUseGradient"] is True
...
def test_delete_logo_regenerates_logo_and_theme(
    ...
    assert refreshed["theme"].get("autogeneratedLogo") is True


def test_update_logo_marks_autogenerated_false(
    running_app, db, branded_community
) -> None:
    """Uploading a custom logo sets `theme.autogeneratedLogo` to False."""
    current_communities.service.update_logo(
        system_identity,
        branded_community.id,
...
def test_service_update_tops_up_generic_theme_for_custom_logo(
    running_app, db, branded_community
) -> None:
    """Missing theme keys on a custom logo get the generic palette, not slug colors."""
    current_communities.service.update_logo(
        system_identity,
        branded_community.id,
        BytesIO(b"custom-logo-bytes"),
    ...
    record = _read_record(branded_community.id)
    record["theme"] = {"enabled": True, "style": {}, "autogeneratedLogo": False}
    ...
    update_data = dict(read_result.data)
    update_data["theme"] = {"enabled": True, "style": {}}
    ...
    for key in expected:
        assert style[key] == expected[key]
    assert refreshed["theme"].get("autogeneratedLogo") is False
    assert style["mainHeaderUseLogo"] is False
    assert style["mainHeaderUseGradient"] is True
