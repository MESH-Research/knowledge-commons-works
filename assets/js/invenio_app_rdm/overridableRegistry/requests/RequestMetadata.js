...
const RequestMetadata = ({ request }) => {
  ...
  // Clear this request from the viewer's unread list when they open it.
  // Unread is already scoped to the current user (sessionStorage from /users/me).
  useEffect(() => {
    const unreadNotifications = getUnreadNotificationsFromStorage();
    const isUnread = unreadNotifications.some(
      (notification) => notification.request_id === request.id
    );
    if (!isUnread) {
      return;
    }
    clearUnreadNotifications({ requestId: request.id }).catch((error) => {
