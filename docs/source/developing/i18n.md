...
From the root of the **translation package** you are working in (each directory has its own `package.json`). Prefer **[pnpm](https://pnpm.io/)** for consistency with the main app: enable Corepack once per Node install (`corepack enable`), then:

```bash
pnpm install
pnpm run extract_messages   # extract strings → JSON / .pot
pnpm run json_to_po         # .json → .po
pnpm run compile_catalog    # .po → .mo
pnpm run init_catalog language en   # new language + package.json list
```
