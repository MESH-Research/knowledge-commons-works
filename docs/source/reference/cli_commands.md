...
- **Theme**: seeds any missing default `theme.style` keys. Theme colors are **slug-derived** when `theme.autogeneratedLogo` is true (or inferred from a matching geopattern logo), or a **site-neutral generic palette** for custom-uploaded logos.
