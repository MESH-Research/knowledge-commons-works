...
#### `invenio kcworks-communities update-index-mapping`

Applies **additive** OpenSearch mapping updates to the live **communities**, **RDM records**, and **RDM drafts** indices, using each index's registered mapping JSON (`communities-communities-v2.0.0` from `invenio-communities`; record/draft mappings from the vendored [`invenio-rdm-records`](customizations.html#invenio-rdm-records) fork).

Fresh installs that run `invenio index init` pick up current mapping files automatically. Run this command when **migrating existing indices** that predate mapping additions (for example `theme.autogeneratedLogo` on communities and on embedded community snapshots under `parent.communities.entries` in record/draft indices).

On existing instances, the live communities index often **drifts** from that file: KCWorks custom fields added via `invenio communities custom-fields init`, locale subfields materialized by dynamic templates (for example `metadata.type.title.de`), and similar. In those cases **`invenio index update communities-communities-v2.0.0` fails** because it compares the entire file and aborts when the diff includes anything other than pure additions.

This command instead:

- reads the registered mapping file and the live index mapping;
- builds a **`put_mapping` patch containing only fields/subfields present in the file but missing on the live index**;
- leaves live-only fields untouched;
- skips fields whose definitions already exist but differ (OpenSearch cannot change them in place), reporting them with `--verbose`.

Use it when deploying mapping changes (for example `theme.autogeneratedLogo`) to **existing** indices. Fresh installs that run `invenio index init` on an empty cluster do not need it.

Named options:

- `--dry-run`: print the `put_mapping` body without writing.
- `--verbose`: print fields skipped because the live mapping differs and cannot be updated in place.

Example — inspect the patch:

```shell
invenio kcworks-communities update-index-mapping --dry-run
```

Example — apply updates on a migrated instance (run **before** branding backfill when the index predates new theme fields):
...
```{note}
On migrated instances whose indices were created before `theme.autogeneratedLogo` was added, run [`update-index-mapping`](#invenio-kcworks-communities-update-index-mapping) first (communities **and** record/draft indices). Otherwise indexing during backfill or record reindex will fail on strict mapping.
```
...
- **Theme**: seeds any missing default `theme.style` keys from the service
  default for that community's logo type (slug-derived palette and header
  flags for geopattern logos; site-neutral palette with
  `mainHeaderUseLogo=false` for user-uploaded logos). With
  `--reset-theme`, any key that differs from the current service default
  is overwritten (colors and header flags).

User-uploaded logos are never overwritten. Admin-customized theme values
are preserved by regular community updates and by backfill unless
`--reset-theme` is passed.
...
- `--reset-theme`: overwrite existing theme values with current service defaults.
