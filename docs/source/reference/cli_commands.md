...
Applies **additive** OpenSearch mapping updates to the live communities index, using the registered `communities-communities-v2.0.0` mapping JSON shipped with `invenio-communities`.
...
Use it when deploying mapping changes (for example `theme.autogeneratedLogo`) to an **existing** index. Fresh installs that run `invenio index init` on an empty cluster do not need it.
...
```{note}
On migrated instances whose communities index was created before `theme.autogeneratedLogo` was added to the mapping, run [`update-index-mapping`](#invenio-kcworks-communities-update-index-mapping) first. Otherwise indexing during backfill will fail on strict mapping.
```
