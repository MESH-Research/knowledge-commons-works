...
KCWorks vendors [MESH-Research/invenio-rdm-records](https://github.com/MESH-Research/invenio-rdm-records) on the `local-working` branch (see `site/kcworks/dependencies/invenio-rdm-records`). The fork tracks upstream InvenioRDM v13 with KCWorks-specific changes.

Record and draft OpenSearch mappings include embedded community theme fields under `parent.communities.entries.theme` (and `entries.parent.theme`). When community theme schema gains new indexed fields (for example `theme.autogeneratedLogo`), add them to the mapping JSON in this fork so `invenio index init` creates correct indices on fresh installs.
